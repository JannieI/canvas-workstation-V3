...
export class canvasProduct {
    ...
    // Generated by the system
    editedBy: string;                       // Last user who edited this record
    editedOn: Date;                         // Date this record was last edited
...
export class CanvasCompany {
    ...
    id: number;                             // Unique ID (added for consistency with other models)
    companyName: string;                    // Company Name
    companyRegistrationNumber: string;      // CIPC reg nr for SA companies
...
export class CanvasContract {
    ...
    // TODO - add id to link to table of Docs
    // Generated by the system
    editedBy: string;                       // Last user who edited this record
    editedOn: Date;                         // Date this record was last edited
    createdBy: string;                      // UserID who created this record
...
// Lookup table for the Type of Contract, ie NDA, Software Licenses
export class CanvasContractType {
    _id?: string;                           // Mongo ID (read only)
    id: number;                             // Unique ID (added for consistency with other models)
    contractType: string;                   // Type of contract
    contractTypeDescription: string;        // Short description of the type of contract
...
// Lookup table for the Status of Contract, ie NDA, Software Licenses
export class CanvasContractStatus {
    _id?: string;                           // Mongo ID (read only)
    id: number;                             // Unique ID (added for consistency with other models)
    contractStatus: string;                 // Status of contract
    contractStatusDescription: string;      // Short description of the Status of contract
...
export class CanvasLicenseUserActivation {
    _id?: string;                           // Mongo ID (read only)
    id: number;                             // Unique ID (added for consistency with other models)
    licenseID: number;                      // FK to License
    userID: string;                         // FK to the User
    activeFrom: string;                     // Date from which user is allowed to use this license
    activeTo: Date;                         // Date license was revoked
...
// productHistory collection.  If this product is referenced in a license,
// that license is upgraded to a new version, the current license is stored in the
// licenseHistory collection and all affected users (as defined in the
// canvasLicense) are updated as well.
// TODO - should be replaced with generic Table Audit trail if Table is marked as
//        sensitive / auditable
export class CanvasProductHistory {
    _id?: string;                           // Mongo ID (read only)
    id: number;                             // Unique ID (added for consistency with other models)
    version: number;                        // Historic version
    name: string;                           // Name of license, also called Product
    description: string;                    // Description
    menuOptionsAllowed: string[];           // Array of menu options allowed (note spelling)
    widgetTypesAllowed: string[];           // Array of widget Types allowed (note spelling)
...
// Describes Licenses that can be purchased by the Client.  A licence can be for a
// predefined Product, and/or custom selected features.  So, a Client can select only
// one Product, or can select only a list of x features.  Or a combination of one Product
...
export class CanvasLicense {
    ...
    paymentFrequency: string;               // Monthly, nnua
    ...
    productID: number;                      // FK to optional Product that is linked to this License
    menuOptionsAllowed: string[];           // Array of menu Additional options allowed (note spelling)
    widgetTypesAllowed: string[];           // Array of Additional widget Types allowed (note spelling)
    datasourcesAllowed: string[];           // Array of Additional Datasources allowed (note spelling)
...
export class Widget {
    ...
    shapeIsItalic: boolean;                 // True if text is italic
    shapeLineHeight: string;                // Line Height: normal, 1.6, 80%
    shapeLineLength: number;                // Length of the line in px
